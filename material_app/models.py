...
class MaterialOverview(models.Model):
    ...
    Owner = models.CharField(max_length=50, null=True, blank=True, verbose_name="負責人")
...
class MaterialItems(models.Model):   ## ← Python 類別名稱 (給程式碼用)
    """物料清單"""
    ...
    class Meta:
        db_table = 'material_app_itemlist'  # ← 資料庫表名 (給資料庫用)
        managed = False  # # ← 不讓 Django 管理資料庫結構
        verbose_name = "物料清單"
...
class TransactionLog(models.Model):
    ...
    SN = models.ForeignKey(
        MaterialItems,  # ✅ 正確:改成 MaterialItems
        on_delete=models.RESTRICT,
        db_column='SN_id',
        verbose_name="料號"
    )
    ...
    def __str__(self):
        return f"{self.LogID} - {self.ActionType} - {self.SN.ItemName}"

class Employee(models.Model):
    """員工基本資料"""
    EmpID = models.CharField(max_length=50, primary_key=True)
    Name = models.CharField(max_length=50)
    DeptID = models.CharField(max_length=50, blank=True, null=True)
